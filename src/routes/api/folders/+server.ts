 ...
 */
function isAncestorOfLabel(id: string, mailboxes: Mailbox[]): boolean {
	const byId = new Map(mailboxes.map((m) => [m.id, m]));
	for (const m of mailboxes) {
		if (!m.name.startsWith(LABEL_PREFIX)) continue;
...
function isLabel(m: Mailbox): boolean {
	return m.name.startsWith(LABEL_PREFIX);
}
	...
	switch (body.action) {
		case 'create': {
			...
			if (body.parentId) {
				...
				if (isLabel(parent)) return json({ success: false, error: 'Cannot nest folders under labels' }, { status: 400 });
			}
		...
		case 'rename': {
			...
			if (isLabel(target)) return json({ success: false, error: 'Rename labels from the Labels settings' }, { status: 400 });
		...
		case 'delete': {
			...
			if (isLabel(target)) return json({ success: false, error: 'Delete labels from the Labels settings' }, { status: 400 });
			if (isAncestorOfLabel(body.id, mailboxes)) {
		...
		case 'move': {
			...
			if (isLabel(target)) return json({ success: false, error: 'Labels live at the root' }, { status: 400 });
