...
const authOdai = {
	authHeader: `Basic ${Buffer.from('odai@odaiameera.com:pass1').toString('base64')}`,
	accountId: 'acc1',
	apiUrl: `${MOCK}/jmap/`,
	sessionState: ''
};
...
try {
	...
	//    working deployments).
	const kept = await resolveCalendarHome(authOdai, 'odai@odaiameera.com');
	expect('constructed home kept when it answers', kept === '/dav/cal/odai%40odaiameera.com/', kept);
	...
	const healed = await resolveCalendarHome(authOdai, 'mismatch@example.com');
	expect(
		'discovery heals a non-conventional principal path',
		healed === '/dav/cal/odai%40odaiameera.com/',
		healed
	);
} catch (err) {
